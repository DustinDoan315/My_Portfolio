...
const ApiIntegration: React.FC<ApiIntegrationProps> = ({ onTextExtracted }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileArray.length > MAX_FILES) {
      setErrorMessage(
        `You can only upload a maximum of ${MAX_FILES} images at once.`
      );
      return;
    }
    ...
    if (oversizedFiles.length > 0) {
      setErrorMessage(
        `The following files exceed the 200KB limit: ${oversizedFiles
  ...
  const generateWordDocument = (extractedTexts: {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.error) {
            ...
            throw new Error(
              `${file.name} - API Error (${response.status}): ${JSON.stringify(
                errorData
              )}`
            );
          }
          ...
          throw new Error(
            `${file.name} - API Error: ${response.status} ${response.statusText}`
          );
        }
      }
